...
const STORAGE_KEYS = {
  lang: 'dc_lang',
  shifts: 'dc_shifts',
  activeShift: 'dc_active_shift', // { startTime: number } or null
};
...
export default function DriverCompanion() {
  ...
  useEffect(() => {
    if (isOnShift && shiftStartTime) {
      saveToStorage(STORAGE_KEYS.activeShift, { startTime: shiftStartTime });
    } else {
      try {
  ...
  const handleSaveShift = () => {
    ...
    const newShift = {
      ...
      distance: Math.round(duration * 22), // estimate
  ...
  const todayHours = todayShifts.reduce((sum, s) => sum + s.duration, 0);
  const todayDistance = todayShifts.reduce((sum, s) => sum + s.distance, 0);
            ...
            <div className={`mb-6 rounded-3xl p-6 ${isOnShift ? 'gradient-shift text-white' : 'bg-white border border-stone-200'}`}>
              ...
              {isOnShift ? (
                <>
                  ...
                  <p className="text-sm text-white/80 mb-6">{t.activeShift}</p>
                  <button
